...
async def generate_response(
    ...
    if sales_script_snippet:
        full_system += (
            ...
            "2. Если значение неизвестно — задай уточняющий вопрос (например: 'Какая дата вам удобна?').\n"
    ...
    def _placeholder_fallback(match: _re.Match) -> str:
        ...
        fallbacks = {
            ...
            "цена_руб": "уточните у менеджера",
            "стоимость": "уточните у менеджера",
            "дата": "какая дата вам удобна",
            ...
            "email": "ваш email",
            "адрес": "наш адрес",
            "размер": "уточните размер",
            "продукт": "эту услугу",
        }
        return fallbacks.get(key, "уточните у менеджера")
